...
------------------------------------------------------------------------------------------------
------------------------ wordle_GetOptimalNextWord_RohitVenkat1
------------------------------------------------------------------------------------------------
if object_id('wordle_GetOptimalNextWord_RohitVenkat1') is not null
begin
    drop proc wordle_GetOptimalNextWord_RohitVenkat1
end

go

CREATE PROC wordle_GetOptimalNextWord_RohitVenkat1
(	@Player char(25)        
...
_________________________________________________________________________________________________
	...
	proc name: 	wordle_GetOptimalNextWord_RohitVenkat1
	author:		Rohit Venkat
...
------------------------------------------------------------------------------------------------
------------------------ wordle_GetOptimalNextWord_RohitVenkat2
------------------------------------------------------------------------------------------------
if object_id('wordle_GetOptimalNextWord_RohitVenkat2') is not null
begin
    drop proc wordle_GetOptimalNextWord_RohitVenkat2
end

go

CREATE PROC wordle_GetOptimalNextWord_RohitVenkat2
(	@Player char(25)        
,	@WordDate date	--in the game of wordle you can only play one word a day, so this is like a WordID
,	@NextGuess char(5) output   --this will return the next word to play
,	@IsDebugRun bit = 0
...
_________________________________________________________________________________________________
	...
	proc name: 	wordle_GetOptimalNextWord_RohitVenkat2
	author:		Rohit Venkat
...
from (
	...
	from (
		select
			...
			NumDistinctLetters,
			Letter,
			LetterProb = cast(count(Num) over(partition by Letter) as numeric) / sum(Num) over()
		from #tmp2 ) t1
	group by Word, NumDistinctLetters) t2
